...
# loader = transforms.Compose([
    # transforms.Resize((imsize1, imsize2)),  # scale imported image
...
style_img = image_loader("images/hand_pro.png").type(dtype)
content_img = image_loader("images/latex_pro.png").type(dtype)
...
def imshow(tensor, title=None):
    ...
    image = image.view(3, imsize1, imsize2)  # remove the fake batch dimension
...
def run_style_transfer(cnn, content_img, style_img, input_img, num_steps=500,
    ...
    cur_best_param = input_param.detach()
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            nonlocal cur_best_score
            nonlocal cur_best_param
            ...
            #cur_score = style_score + content_score
            cur_score = content_score
            cur_best_score = cur_score.data[0] if (cur_score.data[0]<cur_best_score) else cur_best_score
            cur_best_param = input_param.detach() if (cur_score.data[0]<cur_best_score) else cur_best_param
            run[0] += 1
            if run[0] % 50 == 0:
                print("run {}:".format(run))
                print('Style Loss : {:4f} Content Loss: {:4f}'.format(
                    style_score.data[0], content_score.data[0]))
                print('Best loss: {:4f}'.format(cur_best_score))
                print()
    ...
    # a last correction...
    input_param.data.clamp_(0, 1)
    #cur_best_param.data.clamp_(0, 1)

    return cur_best_param.data, cur_best_score

######################################################################
# Finally, run the algorithm

output,score = run_style_transfer(cnn, content_img, style_img, input_img)
print(score)
plt.figure()
...
image = output.clone().cpu()  # we clone the tensor to not do changes on it
image = image.view(3, imsize1, imsize2)  # remove the fake batch dimension
image = unloader(image)
image.save("./images/refined.png")